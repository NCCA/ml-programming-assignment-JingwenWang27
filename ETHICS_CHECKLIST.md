...
- **Training set**: 270 images (90% of total dataset)
- **Validation set**: 30 images (10% of total dataset)
- **Test set**: No separate test set; qualitative evaluation performed on custom photographs
- **Split method**: Random split using PyTorch's `random_split` function at the image level
- **Limitation**: Split is performed at the image level rather than material level. This means the same material may appear in both training and validation sets under different lighting conditions, which could lead to data leakage and potentially overestimate validation performance. Future work should implement material-level splitting to better assess true generalization capability.
...
- **Citation**:
- Textures from ambientCG (https://ambientcg.com/), licensed under CC0 1.0 Universal.
...
- **Data leakage**: Train/validation split performed at image level rather than material level, allowing the same material to appear in both sets under different lighting
...
- Image-level split may lead to overestimated validation performance
...
- **Not implemented**: Geometric transformations (flips, rotations) were not used in this project
...
- Training checkpoints saved every 50 epochs
...
- No geometric data augmentation implemented
- **Data leakage issue**: Train/validation split performed at image level, not material level
- Same material may appear in both training and validation under different lighting
- Validation loss may not accurately reflect true generalization performance
- Model may be learning material-specific features rather than general lighting-to-basecolor mapping
- No separate test set for final evaluation
...
- **Performance claims**: Validation metrics may be overoptimistic due to data leakage
...
- Implement geometric data augmentation (flips, rotations, crops)
...
- [x] No personal/sensitive data used without consent
- [x] Potential biases identified and acknowledged
- [x] Data leakage issue identified and documented
- [x] Generative AI usage fully disclosed
- [x] NCCA coding standards followed where applicable
...
7. I am aware of the limitations and potential risks of this work, including the data leakage issue in the current train/validation split
